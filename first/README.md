...
## 关于P99 算一分钟内全部的
如果数据量sum,
1. sum<100, 那么P99为数据中的最大值。
2. 100<=sum<200, 那么P99为数据中的次大值。
3. 依次...

所以我们可以利用数据量的特征，例如如果确定sum<100，那么我们可以在O(n)时间内找到P99，而不用O(n*log n)排序来找。
...
```
查询1（checkPair）：
输入：主调服务名、被调服务名和时间（分钟粒度, 在计算和保存结果时Long转Date）
...
## 数据的大致解法：
1. 一个线程读取，多个线程处理。或者多个线程各自读取，多个线程各自处理。
2. 每个线程一边读，一边把数据存入特定的数据结构。
3. 每个线程读完1秒个数据后，开始计算并保存结果。
4. 好的数据结构。
5. 减少不必要的运算，优化必要的运算。
6. 重用对象，减少对象的产生，减少GC，减少GC的时间（不发生Full GC）。
## 操作说明
