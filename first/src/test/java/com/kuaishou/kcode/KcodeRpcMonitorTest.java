...
public class KcodeRpcMonitorTest {
    public static void main(String[] args) throws Exception {
        ...
        kcodeRpcMonitor.prepare("D:\\Chrome\\Download\\2kcodeRpcMonitor.data");
        System.out.println("prepare 耗时(ms):" + NANOSECONDS.toMillis(nanoTime() - startNs));

        // 读取checkPair.result文件
        Map<CheckPairKey, Set<CheckPairResult>> checkPairMap = createCheckPairMap("D:\\Chrome\\Download\\checkPair.result");

        // 读取checkResponder.result文件
        Map<CheckResponderKey, com.kuaishou.kcode.CheckResponderResult> checkResponderMap = createCheckResponderMap("D:\\Chrome\\Download\\checkResponder.result");
    ...
    public static void checkPair(KcodeRpcMonitor kcodeRpcMonitor,
        ...
        int checkPairTime = 1000000; // 可以自己修改次数 ->210ms 418ms 318ms
        long cast = 0L;
        while (true) {
    ...
    public static void checkResponder(KcodeRpcMonitor kcodeRpcMonitor,
                                      Map<CheckResponderKey, com.kuaishou.kcode.CheckResponderResult> checkResponderMap) throws ParseException {
        int checkResponderTime = 1000000; // 可以自己修改次数 ->64ms 5250ms 162ms
        long cast = 0L;
        while (true) {
            for (Map.Entry<CheckResponderKey, com.kuaishou.kcode.CheckResponderResult> entry : checkResponderMap.entrySet()) {
                ...
                cast += (nanoTime() - startNs);
                com.kuaishou.kcode.CheckResponderResult checkResponderResult = entry.getValue();
                if (Objects.isNull(result) || !checkResponderResult.equals(new com.kuaishou.kcode.CheckResponderResult(result))) {
